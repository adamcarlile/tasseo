...
function convertedPeriod(period) {
...
function wrapMetric(target, period) {
  url = "summarize(" + target + ", \"" + convertedPeriod(period) + "\", \"avg\", true)";
  return url
}
    ...
    $.ajax({
      ...
      _.each(metrics, function(metric, idx) {
        var metricResult = _.find(metricResults, function(metricResult) {
          console.log({"result": metricResult.target, "comparitor": 'keepLastValue(' + wrapMetric(metric.target, period) + ')'})
          return ('keepLastValue(' + wrapMetric(metric.target, period) + ')').replace(/\W/g, '').split('').sort().join('') == metricResult.target.replace(/\W/g, '').split('').sort().join('');
        });
  ...
  urlForMetrics: function(metrics, period) {
    var targets = _.map(metrics, function(metric) {
      if (this.options.padnulls) {
        return 'target=keepLastValue(' + wrapMetric(metric.target, period) + ')'
      } else {
        return 'target=' + wrapMetric(metric.target, period)
      }
    }, this).join("&");
